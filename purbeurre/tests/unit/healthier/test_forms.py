...
from healthier.forms import FoodQuery, Signin, Login
...
class Test_Signin_form(TestCase):
    ...
    def setUpTestData(cls):
        User.objects.create_user(username="coucou@google.com", first_name="Joe", email="wwww@wwww.www")
    ...
    def test_signin_form_save(self):
        form = Signin(data={"first_name":"Joe", "email":"coucou@google.com", "password1":"1234567BE89", "password2":"1234567BE89"})
        self.assertTrue(form.is_valid())
        self.assertEquals(form.save(), {"email":["coucou@google.com est déjà utilisé par un autre compte, merci d'en utiliser un autre"]})
        ...
        with mock.patch("django.contrib.auth.models.User.save", new=mock_user_save):
            with mock.patch("healthier.forms.login", new=mock_login_true):
                form = Signin(data={"first_name":"Joe", "email":"erzezevze@google.com", "password1":"1234567BE89", "password2":"1234567BE89"})
...
class Test_Login_form(TestCase):
    ...
    def test_log_user(self):
        ...
        with mock.patch("healthier.forms.login", new=mock_login_Validation_Error):
            with mock.patch("django.contrib.auth.forms.AuthenticationForm.clean", new=mock_clean_pass):
                self.assertRaisesMessage(ValidationError, ["E-mail et/ou mot de passe invalides (['none'])"], form.log_user())
